...
DECLARE_SPEC_FUN(       spec_conjure            ); 
DECLARE_SPEC_FUN(       spec_aggro              );
...
//    if ( !str_cmp( name, "spec_dog"		  ) ) return spec_dog;
    ...
    if ( !str_cmp( name, "spec_conjure"           ) ) return spec_conjure;
    if ( !str_cmp( name, "spec_aggro"             ) ) return spec_aggro;
    if ( !str_cmp( name, "spec_forger"            ) ) return spec_forger;
...
bool spec_cast_adept( CHAR_DATA *ch )
{
    ...
    {
    case 0:
	act( "$n utters the word 'somiol'.", ch, NULL, NULL, TO_ROOM );
	spell_armor( skill_lookup( "armor" ), ch->level, ch, victim );
	return TRUE;

    case 1:
	act( "$n utters the word 'nexiv'.", ch, NULL, NULL, TO_ROOM );
	spell_bless( skill_lookup( "bless" ), ch->level, ch, victim );
	return TRUE;

    case 2:
	act( "$n utters the word 'ogguls'.", ch, NULL, NULL, TO_ROOM );
	spell_cure_blindness( skill_lookup( "cure blindness" ),
	    ch->level, ch, victim );
	return TRUE;

    case 3:
	act( "$n utters the word 'elak'.", ch, NULL, NULL, TO_ROOM );
	spell_cure_light( skill_lookup( "cure light" ),
	    ch->level, ch, victim );
	return TRUE;

    case 4:
	act( "$n utters the words 'enilk'.", ch, NULL, NULL, TO_ROOM );
	spell_cure_poison( skill_lookup( "cure poison" ),
	    ch->level, ch, victim );
	return TRUE;

    case 5:
	act( "$n utters the words 'lanimilbus'.", ch, NULL, NULL, TO_ROOM );
	spell_refresh( skill_lookup( "refresh" ), ch->level, ch, victim );
	return TRUE;

    case 6:
	act( "$n utters the words 'noitpurroc'.", ch, NULL, NULL, TO_ROOM );
	spell_refresh( skill_lookup( "fly" ), ch->level, ch, victim );
	return TRUE;

    case 7:
	act( "$n utters the words 'xetrov'.", ch, NULL, NULL, TO_ROOM );
	spell_refresh( skill_lookup( "shield" ), ch->level, ch, victim );
	return TRUE;
...
bool spec_decap(CHAR_DATA *ch)
{
   ...
   for (d = descriptor_list; d != NULL; d = d->next)
   {
       if (!IS_PLAYING(d)
        ...
        if( victim->level == 4 || victim->level == 2 )
	{
	 victim->level = 3;
	 victim->trust = 3;
 	 free_string(victim->clan);
	 victim->clan = str_dup("None");
	 set_title(victim, "FREE CAP");
	 do_chat(ch, "Free cap at temple of the creators!\n\r");
	 do_save(victim, "");
	}
        ...
        if ( !IS_NPC(ch) )
         return FALSE;
        if( victim->hit < 0 && victim->in_room->vnum == 26112)
        {
	 x = number_range(1,100);
	 if( x <= 10 )
	    {
	     do_chat(ch, "Hey!, Come back! You dropped your head! "
			 "You just took it prison style!\n\r");
	     do_tie(ch, victim->name);
	     sprintf(buf, "%s has been decapitated by %s.\n\r", 
	    ...
	    {
	     do_chat(ch, "Another punk gets BEAT down!");	
	     do_escape(victim,"");
             return TRUE;
	    }
        } 
...
bool spec_conjure( CHAR_DATA *ch)
{
   DESCRIPTOR_DATA       *d;
   CHAR_DATA             *victim;
   ...
   for (d = descriptor_list; d != NULL; d = d->next)
   {
     ...
     if( ch->in_room != victim->in_room)
      continue;
     if( IS_SET(ch->in_room->room_flags,ROOM_DARK) && count_mobs(ch) < 15 )
        do_say(ch,"reyals erujnoc nomed nommus!");
     else
     {
      if (number_percent() > 90 && count_mobs(ch) < 15)
      {
       do_say(ch, "thgil enivid lepsid!");
       act("The Mythical Beast summons magic from the darkest depths\n\rof hell to dispel your aura!", 
        ch, NULL, NULL, TO_ROOM);
      }	  
...
bool spec_miner( CHAR_DATA *ch )
{
   ...
   sprintf(name, "Miners Pouch");
   sprintf(sdesc, "#6A Miner's Pouch#n");
   sprintf(ldesc, "#6A Miner's Pouch for storing ores and gems is here#n");
   ...
   container = obj;
   if( ch->level >= 51 && ch->level <= 150 )
      max_gem = number_range(1,2);
   else if( ch->level >= 151 && ch->level <= 250 )
      max_gem = number_range(3,4);
   else if( ch->level >= 251 && ch->level <= 350 )
      max_gem = number_range(5,6);
   else if( ch->level >= 351 && ch->level <= 450 )
      max_gem = number_range(7,8);
   else if( ch->level >= 451 )
      max_gem = number_range(9,10); 
   for( i = 0; i < 10; i++ )
   {
      if(( pObjIndex = get_obj_index( OBJ_VNUM_PROTOPLASM )) == NULL)
      {
...
bool spec_zombie_lord( CHAR_DATA *ch )
{
	...
	for ( ; ; )
	{
	    ...
	    if (victim->in_room == ch->in_room)
	    	...
	    	if      (class_ac(victim)-50 > class_ac(ch)) consider -= 1;
	    	else if (class_ac(victim)+50 < class_ac(ch)) consider += 1;
	    	if      (class_hitroll(victim)+10 < class_hitroll(ch)) consider += 1;
	    	else if (class_hitroll(victim)-10 > class_hitroll(ch)) consider -= 1;
	    	if      (class_damroll(victim)+10 < class_damroll(ch)) consider += 1;
	    	else if (class_damroll(victim)-10 > class_damroll(ch)) consider -= 1;
	    	switch (consider)
	    	{
		    default: break;
		    case 8:
