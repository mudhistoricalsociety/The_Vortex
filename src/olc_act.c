...
AEDIT( aedit_show )
{
    ...
    stcf( ch,	  "Color:    [#%c##%c#n][#%c##%c#n]\n\r",(char )pArea->recall&0xff,(char )pArea->recall&0xff,(char )(pArea->recall>>8)&0xff,(char )(pArea->recall>>8)&0xff);
...
//    argument = one_argument( argument, first );
    ...
    if ( !argument || !argument[0] || !argument[1] || argument[0] == ' ' || argument[1] == ' ' )
    {
	stcf(ch,"Syntax:  colors [title][text]\n\r");
	return FALSE;
    }

    pArea->recall = (int )(argument[0]) + (((int )argument[1]) << 8);

    stcf(ch,"Changed area colors to [#%c##%c#n][#%c##%c#n].\n\r",(char )pArea->recall&0xff,(char )pArea->recall&0xff,(char )(pArea->recall>>8)&0xff,(char )(pArea->recall>>8)&0xff);
    return TRUE;
}
...
OEDIT( oedit_chpon )
{
    ...
    {
	send_to_char( "Syntax: chpoweron [string]\n\r",ch );
	return FALSE;
    }

    if ( !strcmp(argument,"none") )
    {
	free_string(pObj->chpoweron);
	pObj->chpoweron = str_dup("(null)");
	return TRUE;
    }

    free_string(pObj->chpoweron);
    pObj->chpoweron = str_dup(argument);
    return TRUE;
}
...
OEDIT( oedit_chpoff )
{
    ...
    if ( !argument || !argument[0] )
    {
	send_to_char( "Syntax: chpoweroff [string]\n\r",ch );
	return FALSE;
    }

    if ( !strcmp(argument,"none") )
    {
	free_string(pObj->chpoweroff);
	pObj->chpoweroff = str_dup("(null)");
	return TRUE;
    }

    free_string(pObj->chpoweroff);
    pObj->chpoweroff = str_dup(argument);
    return TRUE;
}
...
OEDIT( oedit_chpuse )
{
    ...
    if ( !argument || !argument[0] )
    {
	send_to_char( "Syntax: chpoweruse [string]\n\r",ch );
	return FALSE;
    }

    if ( !strcmp(argument,"none") )
    {
	free_string(pObj->chpoweruse);
	pObj->chpoweruse = str_dup("(null)");
	return TRUE;
    }

    free_string(pObj->chpoweruse);
    pObj->chpoweruse = str_dup(argument);
    return TRUE;
}
...
OEDIT( oedit_vpon )
{
    ...
    if ( !argument || !argument[0] )
    {
	send_to_char( "Syntax: victpoweron [string]\n\r",ch );
	return FALSE;
    }
...
void show_damlist(CHAR_DATA *ch)
{
    ...
    for ( att = 0; weap_type[att] != NULL; att++)
    {
	if ( (att % 21) == 0 )
	    add_buf(buffer,"Noun\n\r");
