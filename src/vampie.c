...
void do_shadowgaze(CHAR_DATA *ch, char *argument  )
{
    ...
    if (IS_AFFECTED(victim, AFF_SHADOWPLANE))
   {
    ...
    if (IS_NPC(victim))
    {
    send_to_char("You cannot Shadow Gaze a mob.\n\r",ch);
    return;
    }
    sprintf(buf,
...
void do_drain( CHAR_DATA *ch, char *argument )
{
   ...
   {
     send_to_char("You cannot drain the life from an arti mob!\n\r", ch);
     return;
   }
   if ((ch->fighting) != NULL)
   {
   ...
   if ( arg[0] == '\0' && (ch->fighting) == NULL)
    ...
    if (IS_NPC(victim))
	{
	  dam = ch->power[DISC_VAMP_DAIM] * 10;
...
void do_shade( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_CLASS(ch, CLASS_VAMPIRE) || (ch->power[DISC_VAMP_OBTE] < 1))
     {	  
        send_to_char("You need level 1 Obtenebration to use this  
power.\n\r",ch);	  
        return;
     }
