...

{
	...
	send_to_char( "  quest quest+ quest- weapon immune\n\r",  ch );
    ...
    if( !str_cmp(arg2,"fblow") )
    {
     ...
     {
      stc("Range is 0 to 100.\n\r",ch);
      return;
     }
     ASKILL(ch, AS_FINAL) = value;
     stc("Value set.\n\r",ch);
     return;
    }
...
void do_paradox( CHAR_DATA *ch, char *argument)  
{
	...
	if (((victim->level) > (ch->level)) && ch->level > 7)
	{
