...
void do_tag( CHAR_DATA *ch, char *argument )
{
    ...
    if( IS_NPC(ch))
     return;
    if( ch->paintball[PAINT_ON] == 0)
    {
     stc("You are not in paintball wars.\n\r", ch);
     return;
    }
    if( arg1[0] == '\0' )
    {
    send_to_char("Who would you like to shoot?\n\r", ch);
     return;
    }
    if( arg2[0] == '\0' )
    {
     send_to_char("Which direction do you wish to shoot?\n\r", ch);
     return;
    } 
    ...
    if( victim->paintball[PAINT_ON] == 0)
    {
     stc("They are not in paintball wars.\n\r", ch);
     return;
    }
    if( victim->in_room->vnum == ROOM_VNUM_TEMPLE)
    {
     stc("Now thats not very fair now is it.\n\r", ch);
     return;
    }
    if( ch->in_room->vnum == ROOM_VNUM_TEMPLE)
    {
     stc("Not while in temple.\n\r", ch);
     return;
    }
    location = ch->in_room;
    shootdirection(ch,victim,3,arg2);
    /* Move them back */
    char_from_room(ch);
...
void shootdirection( CHAR_DATA *ch, CHAR_DATA *victim,int range,char arg[MAX_STRING_LENGTH] )
{
    ...
    if(!found && (range != 0))
     shootdirection(ch, victim, range-1, arg);
    if(!found && (range == 0))
    {
    ...
    if( found && !did_hit)
    {  
     stc("#1Yo#2u m#3is#4se#5d y#6ou#1r m#2ar#3k!#n\n\r", ch);
 stc("#1A bull#2et f#3li#4es ou#5t o#2f now#6her#1e a#5nd sl#2ams #3in#6to t#2he wa#4ll!#n\n\r",victim);
     return;
    }
    if(found && did_hit)
    {
     stc("You just got smacked with a #1p#2a#3i#4n#5t#6b#1a#2l#3l#4!#n\n\r",victim);
     sprintf(buf, "You smacked %s with a #1p#2a#3i#4n#5t#6b#1a#2l#3l#4!#n\n\r",
       victim->name);
     stc( buf, ch);
     ch->paintball[PAINT_KILLS]++;
     victim->paintball[PAINT_DEATHS]--;
     do_goto(victim, "3001");
     sprintf(buf, "%s just smacked %s with a #1p#2a#3i#4n#5t#6b#1a#2l#3l#4!#n\n\r", ch->name,
victim->name);
     do_info(victim, buf);
     if( victim->paintball[PAINT_DEATHS] == 0)
     {
      victim->paintball[PAINT_ON] = 0;
      sprintf(buf, "%s has been knocked out of paintball wars by %s.\n\r", victim->name,
        ch->name);
      do_info(victim,buf);
     }
     WAIT_STATE(ch, 6);
...
void do_paintball( CHAR_DATA *ch, char *argument)
{
 ...
 if( IS_NPC(ch))
   return;
 stc("[    Name    ] [Paintball Status] [Deaths Left] [Kills].\n\r", ch);
 for( d = descriptor_list; d != NULL; d = d->next)
 {
  ...
  continue;
  wch = d->character;
  sprintf(buf, "[%-12s] [%-16d] [%-11d] [%-5d]\n\r", wch->name, wch->paintball[PAINT_ON],
         wch->paintball[PAINT_DEATHS], wch->paintball[PAINT_KILLS]);
  if( !IS_IMMORTAL(wch))
  stc(buf,ch);
 }
 return;
