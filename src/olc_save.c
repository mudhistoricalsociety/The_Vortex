...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
    ...
    fprintf( fp, "%dd%d+%d ",	pMobIndex->hitnodice, 
				...
				pMobIndex->hitplus );
    fprintf( fp, "%dd%d+%d ",	pMobIndex->damnodice, 
				pMobIndex->damsizedice, 
				pMobIndex->damplus );
// gold and exp is not used.
    fprintf( fp, "%d 0\n",
				pMobIndex->gold );
// start_pos auto set to standing.
    fprintf( fp, "%d 8 %d\n",
 ...
 */
	...
	pObjIndex->spectype != 0 && pObjIndex->specpower != 0 )
   {
    fprintf( fp, "Q\n%s~\n%s~\n%s~\n%s~\n%s~\n%s~\n%d %d\n",
...
//    fprintf( fp, "Color %c %c\n",	(char )(pArea->recall & 0xff), (char )((pArea->recall >> 8) & 0xff) );
    fprintf( fp, "Colors %d\n",		pArea->recall);
    fprintf( fp, "End\n\n\n\n" );
